...
8. 修正が完了したら、論理的な単位ごとに分けて **`main` ブランチへ直接
   ...
   みなす**ため、`team-rules/git-workflow.md` の「明示的な指示なしに commit
   しない」「フェーズ実行中はコミットの提案・実行を行わない」は本トライアルには
   適用しない（根拠はルート `CLAUDE.md` の「このリポジトリ自身の git 運用」。
   提案に留めず、実際にコミットすること）
   - ブランチは切らない。`git push` は行わない（ユーザーが内容を確認してから
     実行する）
   - `git add .` は使わずファイルを明示指定し、コミット前に `git status` で
     意図しないファイルが混ざっていないか確認する
   - メッセージは日本語、Conventional Commits の type は英語のまま、
