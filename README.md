...
│   └── reset-phase.sh           ← my-laravel-app/ をフェーズ実行前の状態に戻す
...
`my-laravel-app/` を指定したフェーズの実行前状態に戻すスクリプトです。

```sh
bin/reset-phase.sh <フェーズ番号>
```

Phase 1 は開発サーバー停止・Docker コンテナ/ボリューム破棄・未追跡ファイル削除・
テンプレート状態への復元を行い、完全に白紙の状態まで戻します。Phase 2 以降は
「直前のフェーズ完了時点」まで戻す必要があるため、各フェーズに着手する際に個別の
分岐を実装していく方針です（現時点では Phase 1 のみ実装済み。未実装のフェーズ番号を
指定するとエラーで案内されます）。
