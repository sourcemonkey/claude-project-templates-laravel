...
- **後続フェーズ向けの指示が届く**。フェーズごとに別セッションで走るため、
  あるフェーズのコマンドファイルに「Phase N ではこう実装する」と書いても、
  Phase N のセッションはそのファイルを読まない。2026-08-12 のトライアルでは、
  ロール変更を明示代入で書く指示が Phase 2 のコマンドファイルにだけ存在し、
  Phase 3 が Mass Assignment で実装して**黙って無視される**バグを作った
  （Phase 4 のテストで検出）
...
出力は Markdown の表（stdout。セッションのパスと警告は stderr）で、そのまま issue に
貼れます。**トライアルセッション自身は集計しません**（マーカーを出すだけ）。数値が要るのは
フェーズ実行中ではなくトライアル全体を振り返るときで、報告中に集計してもその報告自身の
消費を含められず、後から `--all` で出す値と必ずズレるためです。

| 列 | 意味 |
|---|---|
| API 呼び出し | その区間の API リクエスト数 |
| ツール | その区間で使ったツールの延べ回数 |
| 密度 | ツール ÷ API 呼び出し。**同じ仕事でもここが半分になれば消費は倍になる**（下記） |
...
> **消費を左右するのは「密度」です。** API はステートレスで毎回全履歴を再送するため、
> コストは「呼び出し数 × その時点の文脈」で決まります。同じ Phase 2 を 2 回実行した実測では、
> ツール使用回数が 91 対 110 とほぼ同じなのに、密度 2.60 のランが 3.9M、密度 1.83 のランが
> 7.0M でした。**差は束ね方だけです。** 密度 1.00（毎回 1 個ずつ）まで落ちたランもあり、
> 手順書や docs を削る施策より桁が大きい変動要因になっています。
...
2. ユーザーが選んだ 1 件について、「選択肢」と「推奨」を示して判断を仰ぐ。
   ...
   **`状態: 未分類` の件は、まず切り分けから始める**（`--model sonnet` の観測記録のため
   ...
   | 仕様の自由度 | 記述が無いのは意図的で、どう埋めても実行結果が変わらない | 手順書は変更しない |
